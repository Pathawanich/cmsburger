<?php
add_filter('woocommerce_product_add_to_cart_text', 'translate_add_to_cart_button_text');

add_filter('gettext', 'translate_view_cart_btn');

add_filter('gettext', 'translate_checkout_btn');
...
add_filter( 'gettext', 'translate_update_cart_text', 20, 3 );

add_filter( 'gettext', 'translate_cart_totals_text', 20, 3 );
...
function translate_add_to_cart_button_text() {
...
function translate_update_cart_text( $translated, $text, $domain ) {
...
function translate_cart_totals_text( $translated, $text, $domain ) {
...
function translate_checkout_btn($checkout_btn){
  $checkout_btn= str_ireplace('Checkout', 'Til kassen', $checkout_btn);
  return $checkout_btn;
}

function translate_view_cart_btn($view_cart_btn){
  $view_cart_btn= str_ireplace('View cart', 'Vis handlekurv', $view_cart_btn);
  return $view_cart_btn;
}

function translate_woocommerce_button_proceed_to_checkout() {
